<x-app-layout>
        ...
        <div class="px-2 max-w-7sm gap-8 justify-center mx-auto sm:px-6 lg:px-8 md:flex lg:flex">
                        ...
                        <div class="p-12" style="width:100%">
                            <form method="POST" action="{{ route('merk.store') }}">
                                ...
                                <div class="mb-5">
                                    ...
                                    <span class="text-sm m-l text-red-500">{{ $errors->first('merk') }}</span>
                                </div>
                                <div class="mb-5">
                                    <label for="lantai"
            ...
            <div class="bg-white my-2 overflow-hidden shadow-lg w-full rounded-lg lg:rounded-lg md:rounded-lg">
                <div class="p-4 text-gray-900 w-full">
                    ...
                    <div class="flex justify-center">
                        <div class="p-12" style="width:100%">
                            <table class="table table-bordered" id="merk-datatable">
                                <thead>
                                    <tr>
                                        <th class="w-7">No.</th>
                                        <th>Merk</th>
                    ...
                    <div class="w-full md:w-1/2 relative bg-white rounded-lg shadow mx-5">
                        ...
                        <form method="POST"  id="formSourceModal">
                            ...
                            <div class="flex flex-col  p-4 space-y-6">
                                ...
                                <div>
                                    <label for="text"
                                        class="block mb-2 text-sm font-medium text-gray-900">Merk</label>
                                    <input type="text" id="merks" name="merk"
                                        ...
                                        placeholder="Masukan lantai disini...">
                                </div>
                            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#merk-datatable').DataTable({
                ...
                initComplete: function() {
                    ...
                    $('#merk-datatable thead th').css('text-align', 'center');
                },
                columns: [{
                    data: 'no',
                    ...
                    render: (data) => {
                        let editUrl =
                            `<button type="button" data-id="${data.id}"
                                                        data-modal-target="sourceModal" data-merk="${data.merk}"
                                                        onclick="editSourceModal(this)"
                                                        class="bg-amber-500 hover:bg-amber-600 px-3 py-1 rounded-md text-xs text-white">
                                                       <i class="fas fa-edit"></i>
                                                    </button>`;
                        let deleteUrl =
                            `<button onclick="return merkDelete('${data.id}','${data.merk}')" class="bg-red-500 hover:bg-bg-red-300 px-3 py-1 rounded-md text-xs text-white"><i class="fas fa-trash"></i></button>`;
        ...
        const editSourceModal = (button) => {
            ...
            let url = "{{ route('merk.update', ':id') }}".replace(':id', id);
            console.log(url);
            let status = document.getElementById(modalTarget);
            document.getElementById('title_source').innerText = `Update Merk ${merk}`;
        ...
        const merkDelete = async (id, merk) => {
            let tanya = confirm(`Apakah anda yakin untuk menghapus merk ${merk} ?`);
            if (tanya) {
                await axios.post(`/merk/${id}`, {
                        '_method': 'DELETE',
